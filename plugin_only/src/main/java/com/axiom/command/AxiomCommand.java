...
public class AxiomCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length == 0) {
            sender.sendMessage("§6§l=== AXIOM КОМАНДЫ ===");
            sender.sendMessage("§bОсновные: §7/axiom nation | /axiom claim | /axiom unclaim");
            sender.sendMessage("§bЭкономика: §7/axiom economy | /axiom banking | /axiom stock | /axiom wallet");
            sender.sendMessage("§bДипломатия: §7/axiom diplomacy | /axiom treaty | /axiom alliance");
            sender.sendMessage("§bВойны: §7/axiom war | /axiom raid | /axiom siege | /axiom conquest");
            sender.sendMessage("§bОбщество: §7/axiom religion | /axiom culture | /axiom education | /axiom crime");
            sender.sendMessage("§bПрочее: §7/axiom pvp | /axiom city | /axiom resources | /axiom stats");
            sender.sendMessage("§7Используйте §b/axiom nation §7для открытия главного меню!");
            return true;
        }
        ...
        switch (sub) {
            ...
            case "nation":
                if (args.length >= 2 && args[1].equalsIgnoreCase("create")) {
                    return new CreateNationCommand(plugin, plugin.getNationManager(), plugin.getPlayerDataManager())
            ...
            case "city":
                ...
                if (args.length >= 2 && args[1].equalsIgnoreCase("found")) {
                    ...
                    if (me.isEmpty()) { sender.sendMessage("§cВы не в нации."); return true; }
                    if (args.length < 3) { sender.sendMessage("§cИспользование: /axiom city found <name>"); return true; }
                    var ch = ((Player) sender).getLocation().getChunk();
                    String key = ch.getWorld().getName() + ":" + ch.getX() + ":" + ch.getZ();
                    try {
                        plugin.getCityGrowthEngine().createCity(me.get().getId(), args[2], key);
                        sender.sendMessage("§aГород основан.");
                    } catch (Exception e) { sender.sendMessage("§cОшибка: " + e.getMessage()); }
                    return true;
                }
                plugin.openCitiesMenu((Player) sender);
                return true;
            case "economy":
                return handleEconomy(sender, command, label, args);
            case "claim":
                // Pass only "claim" as args[0] to ClaimCommand
                return new ClaimCommand(plugin, plugin.getNationManager()).onCommand(sender, command, label, new String[]{"claim"});
            case "unclaim":
                // Pass only "unclaim" as args[0] to UnclaimCommand
                return new UnclaimCommand(plugin, plugin.getNationManager()).onCommand(sender, command, label, new String[]{"unclaim"});
            case "pvp":
                return handlePvp(sender, args);
            case "diplomacy":
                return handleDiplomacy(sender, args);
            case "religion":
                return handleReligion(sender, args);
            case "save":
                return handleSave(sender);
            case "citizenship":
                return handleCitizenship(sender, args);
            case "election":
                return handleElection(sender, args);
            case "backup":
                return handleBackup(sender, args);
            case "restore":
                return handleRestore(sender, args);
            case "tutorial":
                return handleTutorial(sender, args);
            case "export":
            ...
            case "dashboard":
            case "stats":
                return handleDashboard(sender, args);
            case "war":
                return handleWar(sender, args);
            case "banking":
                return handleBanking(sender, args);
            case "stock":
                return handleStock(sender, args);
            case "wallet":
                return handleWallet(sender, args);
            case "resources":
                return handleResources(sender, args);
            case "raid":
                return handleRaid(sender, args);
            case "siege":
                return handleSiege(sender, args);
            case "culture":
                return handleCulture(sender, args);
            case "espionage":
            case "spy":
                return handleEspionage(sender, args);
            case "advanced":
            case "features":
                return handleAdvancedFeatures(sender, args);
            case "recipeint":
            case "rit":
                return handleRecipeIntegration(sender, args);
            case "modenhance":
            case "mie":
                return handleModEnhancement(sender, args);
            case "modpackm":
            case "modpackmanager":
                return handleModPackManager(sender, args);
            default:
                sender.sendMessage("§cНеизвестная команда. Используйте §b/axiom §7для списка команд.");
                return true;
        }
    }
    ...
    private boolean handleRecipeIntegration(CommandSender sender, String[] args) {
        if (!sender.hasPermission("axiom.recipeint.use")) {
            sender.sendMessage(ChatColor.RED + "Недостаточно прав.");
            return true;
        }

        // Pass the command to the dedicated RecipeIntegrationCommand handler
        // Since we can't directly call another command executor, we'll implement basic functionality here
        if (args.length < 2) {
            sender.sendMessage("§6§l=== RECIPE INTEGRATION ===");
            sender.sendMessage("§b/axiom recipeint list §7- список интеграций");
            sender.sendMessage("§b/axiom recipeint recipes §7- рецепты интеграции");
            sender.sendMessage("§b/axiom recipeint stats §7- статистика интеграции");
            sender.sendMessage("§b/axiom recipeint search <ключевое слово> §7- поиск рецептов");
            return true;
        }
        ...
        switch (subCommand.toLowerCase()) {
            case "list":
                ...
                var recipes = plugin.getRecipeIntegrationService().getAllIntegrationRecipes();
                sender.sendMessage("§6§l=== ДОСТУПНЫЕ ИНТЕГРАЦИИ ===");
                for (var entry : recipes.entrySet()) {
                    sender.sendMessage("§a" + entry.getKey() + " §7- " + entry.getValue().size() + " рецептов");
                }
                break;
            case "stats":
                // Show basic statistics
                var stats = plugin.getRecipeIntegrationService().getIntegrationStatistics();
                sender.sendMessage("§6§l=== СТАТИСТИКА ИНТЕГРАЦИИ ===");
                sender.sendMessage("§7Включено интеграций: §f" + stats.get("enabledIntegrations"));
                sender.sendMessage("§7Всего рецептов: §f" + stats.get("totalRecipeMappings"));
                sender.sendMessage("§7Использований рецептов: §f" + stats.get("totalRecipeUsage"));
                break;
            default:
                sender.sendMessage("§cНеизвестная подкоманда. Используйте: list, stats");
        }
        return true;
    }
    
    private boolean handleModEnhancement(CommandSender sender, String[] args) {
        if (!sender.hasPermission("axiom.modenhance.use")) {
            sender.sendMessage(ChatColor.RED + "Недостаточно прав.");
            return true;
        }

        if (args.length < 2) {
            sender.sendMessage("§6§l=== MOD INTEGRATION ENHANCEMENT ===");
            sender.sendMessage("§b/axiom modenhance mappings §7- карты материалов модов");
            sender.sendMessage("§b/axiom modenhance compat <материал1> <материал2> §7- совместимость");
            sender.sendMessage("§b/axiom modenhance stats §7- статистика интеграции");
            return true;
        }
        ...
        switch (subCommand.toLowerCase()) {
            case "mappings":
                var mappingStats = plugin.getModIntegrationEnhancementService().getMaterialMappingStats();
                sender.sendMessage("§6§l=== КАРТЫ МАТЕРИАЛОВ МОДОВ ===");
                sender.sendMessage("§7Всего сопоставлений: §f" + mappingStats.get("totalModMaterialMappings"));
                break;
            case "stats":
                var recipeStats = plugin.getRecipeIntegrationService().getIntegrationStatistics();
                var modStats = plugin.getModIntegrationEnhancementService().getMaterialMappingStats();
                sender.sendMessage("§6§l=== СТАТИСТИКА ИНТЕГРАЦИИ МОДОВ ===");
                sender.sendMessage("§7Сопоставлений материалов: §f" + modStats.get("totalModMaterialMappings"));
                sender.sendMessage("§7Рецептов интеграции: §f" + recipeStats.get("totalRecipeMappings"));
                sender.sendMessage("§7Включено интеграций: §f" + recipeStats.get("enabledIntegrations"));
                break;
            default:
                sender.sendMessage("§cНеизвестная подкоманда. Используйте: mappings, stats");
        }
        return true;
    }
    
    private boolean handleModPackManager(CommandSender sender, String[] args) {
        if (!sender.hasPermission("axiom.modpackmanager.use")) {
            sender.sendMessage(ChatColor.RED + "Недостаточно прав.");
            return true;
        }

        if (args.length < 2) {
            sender.sendMessage("§6§l=== MOD PACK MANAGER ===");
            sender.sendMessage("§b/axiom modpackm list §7- список модпаков");
            sender.sendMessage("§b/axiom modpackm info <id> §7- информация о модпаке");
            sender.sendMessage("§b/axiom modpackm enable <id> §7- активировать модпак");
            sender.sendMessage("§b/axiom modpackm stats §7- статистика модпаков");
            return true;
        }
        ...
        switch (subCommand.toLowerCase()) {
            case "list":
                var modPacks = plugin.getModPackManagerService().getAvailableModPacks();
                sender.sendMessage("§6§l=== ДОСТУПНЫЕ МОДПАКИ ===");
                for (var modPack : modPacks) {
                    String status = sender instanceof Player ? 
                        (plugin.getModPackManagerService().getActiveModPackForNation(
                            plugin.getNationManager().getNationOfPlayer(((Player)sender).getUniqueId())
                                .map(n -> n.getId()).orElse(""))).equals(modPack.id) ? 
                         ChatColor.GREEN + "[АКТИВЕН]" : ChatColor.GRAY + "[НЕ АКТИВЕН]") : 
                         ChatColor.GRAY + "[СЕРВЕР]";
                    sender.sendMessage("§a" + modPack.name + " " + status);
                }
                break;
            case "stats":
                var stats = plugin.getModPackManagerService().getModPackStatistics();
                sender.sendMessage("§6§l=== СТАТИСТИКА МОДПАКОВ ===");
                sender.sendMessage("§7Всего модпаков: §f" + stats.get("totalModPacks"));
                sender.sendMessage("§7Наций с модпаками: §f" + stats.get("activeNations"));
                break;
            default:
                sender.sendMessage("§cНеизвестная подкоманда. Используйте: list, stats");
        }
