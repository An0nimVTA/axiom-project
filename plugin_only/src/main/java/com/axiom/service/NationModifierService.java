...
public class NationModifierService {
    ...
    public synchronized String addModifier(String nationId, String type, String effect, double value, long durationMinutes) {
        ...
        org.bukkit.Bukkit.getScheduler().runTask(plugin, () -> {
            ...
            if (n != null) {
                ...
                String effectName = effect.equals("bonus") ? "§aБонус" : "§cШтраф";
                String msg = effectName + " §f" + typeName + " §7(" + String.format("%.0f", value * 100) + "%) - §7" + durationMinutes + " мин";
    ...
    public synchronized double getModifierValue(String nationId, String type) {
        ...
        for (Modifier m : mods) {
            if (m.type.equals(type) && m.expiresAt > now) {
                if (m.effect.equals("bonus")) total *= (1.0 + m.value);
    ...
    private void cleanupExpired() {
        ...
        for (var entry : nationModifiers.entrySet()) {
            ...
            if (!expired.isEmpty()) {
                ...
                org.bukkit.Bukkit.getScheduler().runTask(plugin, () -> {
                    com.axiom.model.Nation n = plugin.getNationManager().getNationById(entry.getKey());
