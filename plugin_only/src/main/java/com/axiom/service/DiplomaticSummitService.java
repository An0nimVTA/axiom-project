    ...
    private void processSummits() {
        ...
        for (Map.Entry<String, DiplomaticSummit> e : activeSummits.entrySet()) {
            ...
            if (elapsed >= summit.durationMinutes * 60_000L) {
                ...
                for (String participantId : summit.participantNations) {
                    for (String otherId : summit.participantNations) {
                        if (!participantId.equals(otherId)) {
                            try {
                                ...
                                if (p != null && o != null) {
                                    plugin.getDiplomacySystem().setReputation(p, o, 5);
                                    plugin.getDiplomacySystem().setReputation(o, p, 5);
                                }
                            } catch (Exception ignored) {}
                        }
                    }
