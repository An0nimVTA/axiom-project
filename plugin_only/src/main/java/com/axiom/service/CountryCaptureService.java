...
import com.axiom.model.CityCore;
import com.axiom.model.CountryCore;
...
public class CountryCaptureService {
    ...
    private boolean isNearEnemyCore(Chunk chunk, String enemyNationId) {
        ...
        if (enemyCore != null) {
            ...
            if (coreLoc != null) {
                ...
                int distance = (int) coreLoc.distance(chunk.getWorld().getBlockAt(
                    chunk.getX() * 16, coreLoc.getBlockY(), chunk.getZ() * 16).getLocation());
    ...
    /**
     * Проверить, находится ли страна под осадой
     */
    public boolean isUnderSiege(String nationId) {
        return siegeTimers.containsKey(nationId);
    }
    
    /**
     * Получить укрепленные чанки для нации
     */
    public Set<Chunk> getTrenchChunksForNation(String nationId) {
        Set<Chunk> result = new HashSet<>();
        for (Map.Entry<Chunk, String> entry : trenchChunks.entrySet()) {
    ...
    /**
     * Проверить, контролирует ли нация ядро страны
     */
    public boolean doesNationControlCountryCore(String nationId, String countryCoreId) {
        CountryCore core = countryCores.get(countryCoreId);
        return core != null && core.getId().equals(nationId);
    }
    
    /**
     * Проверить, контролирует ли нация ядро города
     */
    public boolean doesNationControlCityCore(String nationId, String cityCoreId) {
        CityCore core = cityCores.get(cityCoreId);
        return core != null && core.getNationId().equals(nationId);
    }
    
    /**
     * Повредить ядро страны
     */
    public boolean damageCountryCore(String countryCoreId, int damage) {
        CountryCore core = countryCores.get(countryCoreId);
        if (core == null) return false;
        
        boolean destroyed = core.damageCountry(damage);
        if (destroyed) {
            onCountryCaptured(countryCoreId);
        }
        return destroyed;
    }
    
    /**
     * Повредить ядро города
     */
    public boolean damageCityCore(String cityCoreId, int damage) {
        CityCore core = cityCores.get(cityCoreId);
        if (core == null) return false;
        
        boolean destroyed = core.damage(damage);
        if (destroyed) {
            onCityCaptured(cityCoreId);
        }
        return destroyed;
    }
    
    /**
     * Захват чанка для укреплений
     */
    public boolean captureChunkForTrenches(Chunk chunk, String nationId, Player player) {
        String ownerNation = getChunkOwner(chunk);
        ...
        if (ownerNation != null && !ownerNation.equals(nationId)) {
            ...
            if (isNearEnemyCore(chunk, ownerNation)) {
                ...
                // Отправляем сообщения игроку
                player.sendMessage(ChatColor.GREEN + "Вы создали укрепления (окопы) в чанке " + 
                    chunk.getX() + "," + chunk.getZ() + "!");
                    
                return true;
            } else {
                player.sendMessage(ChatColor.RED + "Вы можете создавать укрепления только рядом с вражеским ядром!");
                return false;
            }
        } else if (ownerNation == null) {
            // Нейтральная территория
            trenchChunks.put(chunk, nationId);
            player.sendMessage(ChatColor.GREEN + "Вы захватили чанк " + chunk.getX() + "," + chunk.getZ() + " для укреплений!");
            return true;
        } else {
            // Свой чанк
            trenchChunks.put(chunk, nationId);
            player.sendMessage(ChatColor.YELLOW + "Вы укрепляете чанк " + chunk.getX() + "," + chunk.getZ() + "!");
            return true;
        }
    }
     ...
     * Проверка, рядом ли чанк с ядром нации
     */
    public boolean isNearEnemyCore(Chunk chunk, String enemyNationId) {
        // Проверяем, рядом ли чанк с каким-либо ядром вражеской нации
        CountryCore enemyCore = countryCores.get(enemyNationId);
        if (enemyCore != null) {
            Location coreLoc = enemyCore.getCapitalLocation();
            if (coreLoc != null) {
                ...
                if (distance <= 160) { // 10 чанков * 16 блоков = 160 блоков
                    return true;
                }
            }
        }
    ...
    public boolean beginSiege(String attackerNationId, String defenderNationId) {
        ...
        String defenderNationName = getCountryCore(defenderNationId) != null ? 
            getCountryCore(defenderNationId).getNationName() : defenderNationId;
        
        broadcastMessage("§c§lОСАДА!", "§fНация §e" + defenderNationName + " §fнаходится под осадой нацией §b" + attackerNationId + "§f!");
        
        return true;
    }
    
    /**
     * Проверить, под осадой ли страна
     */
    public boolean isUnderSiege(String nationId) {
        return siegeTimers.containsKey(nationId);
    }
     ...
     */
    private void broadcastMessage(String title, String message) {
        for (org.bukkit.entity.Player player : org.bukkit.Bukkit.getOnlinePlayers()) {
            player.sendMessage(message);
            player.sendTitle(title, message, 10, 70, 20);
        }
