...
public class SiegeService {
    ...
    private void completeSiege(Siege s) {
        ...
        com.axiom.model.City city = plugin.getCityGrowthEngine().getCitiesOf(s.defenderNationId).stream()
            .filter(c -> c.getId().equals(s.cityId))
            .findFirst()
            .orElse(null);
        if (city != null) {
            // Transfer city
        ...
        Nation attacker = nationManager.getNationById(s.attackerNationId);
        Nation defender = nationManager.getNationById(s.defenderNationId);
        if (attacker != null) attacker.getHistory().add("Город " + s.cityId + " захвачен!");
        if (defender != null) defender.getHistory().add("Город " + s.cityId + " потерян!");
        try {
    ...
    private void loadAll() {
        ...
        for (File f : files) {
            try (Reader r = new InputStreamReader(Files.newInputStream(f.toPath()), StandardCharsets.UTF_8)) {
                ...
                Siege s = new Siege();
                s.cityId = o.get("cityId").getAsString();
                s.attackerNationId = o.get("attackerNationId").getAsString();
                s.defenderNationId = o.get("defenderNationId").getAsString();
                s.progress = o.get("progress").getAsDouble();
                s.startTime = o.get("startTime").getAsLong();
                if (s.progress < 100) activeSieges.put(s.cityId, s);
            } catch (Exception ignored) {}
        }
    }
    ...
    public synchronized Map<String, Object> getSiegeStatistics(String nationId) {
        ...
        for (Siege s : activeSieges.values()) {
            if (s.attackerNationId.equals(nationId)) {
        ...
        for (Siege s : activeSieges.values()) {
            if (s.defenderNationId.equals(nationId)) {
