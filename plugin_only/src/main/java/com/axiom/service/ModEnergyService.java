...
public class ModEnergyService {
    ...
    public boolean isEnergyGenerator(Block block) {
        return modIntegration.isEnergyBlock(block);
    }
    ...
    private void updateEnergyProduction() {
        for (Nation n : plugin.getNationManager().getAll()) {
            ...
            if (modIntegration.isIndustrialUpgradeAvailable()) {
                energy *= 1.5; // +50% efficiency
            }
            if (modIntegration.isQuantumGeneratorsAvailable()) {
    ...
    public Map<String, Object> getEnergyStatistics(String nationId) {
        ...
        stats.put("hasIndustrialUpgrade", modIntegration.isIndustrialUpgradeAvailable());
        stats.put("hasQuantumGenerators", modIntegration.isQuantumGeneratorsAvailable());
        ...
        if (n != null) {
            ...
            if (plugin.getCityGrowthEngine() != null) {
                estimatedConsumption += plugin.getCityGrowthEngine().getCitiesOf(nationId).size() * 50.0;
            }
    ...
    public double getMaximumEnergyPotential(String nationId) {
        ...
        if (modIntegration.isIndustrialUpgradeAvailable()) multiplier *= 1.5;
        if (modIntegration.isQuantumGeneratorsAvailable()) multiplier *= 2.0;
        if (modIntegration.isIEAvailable()) multiplier *= 1.3;
    ...
    public boolean canExportEnergy(String nationId, double amount) {
        ...
        double surplus = stats.containsKey("surplus") ? (Double) stats.get("surplus") : 0.0;
    ...
    public synchronized Map<String, Object> getGlobalModEnergyStatistics() {
        ...
        stats.put("hasIndustrialUpgrade", modIntegration.isIndustrialUpgradeAvailable());
        stats.put("hasQuantumGenerators", modIntegration.isQuantumGeneratorsAvailable());
        stats.put("hasIEAvailable", modIntegration.isIEAvailable());
