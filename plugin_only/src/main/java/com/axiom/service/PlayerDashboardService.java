...
public class PlayerDashboardService {
    ...
    public void updateDashboard(Player player) {
        ...
        if (nation != null) {
            ...
            addLine(scoreboard, objective, "§f│ §b🗺️ Территории: §b" + nation.getClaimedChunkKeys().size(), lineNumber--);
            addLine(scoreboard, objective, " ", lineNumber--);
            ...
            int totalTechs = plugin.getTechnologyTreeService().getAllTechs().size();
            long unlockedTechs = plugin.getTechnologyTreeService().getUnlockedTechs(nationId).size();
            ...
            Set<String> detectedMods = plugin.getModIntegrationService().getDetectedMods();
            if (!detectedMods.isEmpty()) {
                int warfareMods = plugin.getModWarfareService().getAvailableWarfareModsCount();
                boolean hasIndustrial = plugin.getModIntegrationService().hasIndustrialMods();
                boolean hasEnergy = plugin.getModIntegrationService().hasEnergyMods();
            ...
            addLine(scoreboard, objective, "§f│ §a🤝 Союзников: §f" + nation.getAllies().size(), lineNumber--);
            addLine(scoreboard, objective, "§f│ §c⚔️ Врагов: §f" + nation.getEnemies().size(), lineNumber--);
            
            // Reputation (if any)
            if (!nation.getReputation().isEmpty()) {
                int avgRep = (int) nation.getReputation().values().stream()
    ...
    private boolean isNationAtWar(Nation nation) {
        if (nation.getEnemies().isEmpty()) return false;
        
        for (String enemyId : nation.getEnemies()) {
            if (plugin.getDiplomacySystem().isAtWar(nation.getId(), enemyId)) {
                return true;
            }
        }
        return false;
