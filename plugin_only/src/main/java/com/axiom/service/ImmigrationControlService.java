...
public class ImmigrationControlService {
    ...
    public synchronized String approveImmigration(String nationId, String playerId, UUID approverId) {
        ...
        if (!plugin.getRolePermissionService().hasPermission(approverId, "approve_immigration")) return "Нет прав.";
        ...
        plugin.getMigrationService().migratePlayer(playerUUID, req.fromNationId, req.toNationId);
        requests.remove(req);
