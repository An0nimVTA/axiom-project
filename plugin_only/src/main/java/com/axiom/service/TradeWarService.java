...
public class TradeWarService {
    ...
    public synchronized String startTradeWar(String nationA, String nationB, double tariff) {
        ...
        return "Торговая война начата (пошлина: " + tariff + "%)";
    }
    ...
    public synchronized String retaliate(String nationId, String targetId, double tariff) {
        ...
        saveTradeWar(war);
        return "Ответная пошлина установлена: " + tariff + "%";
    }

    private void processTradeWars() {
    ...
    private void loadAll() {
        ...
        for (File f : files) {
            try (Reader r = new InputStreamReader(Files.newInputStream(f.toPath()), StandardCharsets.UTF_8)) {
                ...
                TradeWar war = new TradeWar();
                war.nationA = o.get("nationA").getAsString();
                war.nationB = o.get("nationB").getAsString();
                war.tariffA = o.get("tariffA").getAsDouble();
                war.tariffB = o.get("tariffB").getAsDouble();
                war.startedAt = o.get("startedAt").getAsLong();
