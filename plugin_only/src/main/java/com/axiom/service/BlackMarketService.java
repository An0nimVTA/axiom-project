...
public class BlackMarketService {
    ...
    public synchronized String buyFromBlackMarket(UUID buyerId, String item, int quantity, double pricePerUnit) {
        ...
        if (plugin.getWalletService().getBalance(buyerId) < totalCost) return "Недостаточно средств.";
        plugin.getWalletService().withdraw(buyerId, totalCost);
        // Add resources to buyer's nation
        String nationId = plugin.getPlayerDataManager().getNation(buyerId);
        if (nationId != null) {
            plugin.getResourceService().addResource(nationId, item, quantity);
        }
