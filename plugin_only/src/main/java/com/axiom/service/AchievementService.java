            ...
            org.bukkit.Bukkit.getScheduler().runTask(plugin, () -> {
                ...
                if (player != null && player.isOnline()) {
                    ...
                    if (rarity.equals("LEGENDARY") || rarity.equals("MYTHIC")) {
                        plugin.getVisualEffectsService().playLegendaryAchievementEffect(player, achievementName, rarity);
                    } else {
                        // Standard achievement effect
                        player.sendTitle("§6§l[ДОСТИЖЕНИЕ]", "§e" + achievementName, 10, 80, 20);
                        plugin.getVisualEffectsService().sendActionBar(player, "§6🏆 Достижение разблокировано: §e" + achievementName);
