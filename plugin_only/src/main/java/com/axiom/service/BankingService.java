...
public class BankingService {
    ...
    public synchronized String repayLoan(String borrowerId, String loanId, double amount) throws IOException {
        ...
        Loan loan = loans.stream().filter(l -> l.lenderNationId.equals(loanId) || l.borrowerNationId.equals(borrowerId)).findFirst().orElse(null);
    ...
    private void loadAll() {
        ...
        for (File f : files) {
            try (Reader r = new InputStreamReader(Files.newInputStream(f.toPath()), StandardCharsets.UTF_8)) {
                ...
                if (loan.remaining > 0 && loan.dueAt > System.currentTimeMillis()) {
