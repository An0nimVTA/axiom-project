...
public class ClimateService {
    ...
    public synchronized Map<String, Object> getClimateStatistics(String nationId) {
        ...
        stats.put("totalChunks", n.getClaimedChunkKeys().size());
        
        // Climate stability rating
        double stability = affectedChunks > 0 ? 
            Math.max(0, 100.0 - (affectedChunks / (double) n.getClaimedChunkKeys().size()) * 100.0) : 100.0;
    ...
    public synchronized String applyClimateChange(String chunkKey, Biome newBiome) {
        ...
        if (change.originalBiome == null) {
            // Store original biome
            org.bukkit.Chunk chunk = plugin.getServer().getWorlds().get(0).getChunkAt(
                Integer.parseInt(chunkKey.split(":")[0]), 
                Integer.parseInt(chunkKey.split(":")[1]));
    ...
    public synchronized Map<String, Object> getGlobalClimateStatistics() {
        ...
        for (com.axiom.model.Nation n : plugin.getNationManager().getAll()) {
            ...
            if (affected > 0) {
                ...
                double stability = n.getClaimedChunkKeys().size() > 0 ? 
                    Math.max(0, 100.0 - (affected / (double) n.getClaimedChunkKeys().size()) * 100.0) : 100.0;
