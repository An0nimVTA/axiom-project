...
public class StockMarketService {
    ...
    public synchronized String createCorporation(String nationId, String name, String type) throws IOException {
        ...
        org.bukkit.entity.Player firstPlayer = owner.getCitizens().stream()
    ...
    private void loadAll() {
        ...
        for (File f : files) {
            try (Reader r = new InputStreamReader(Files.newInputStream(f.toPath()), StandardCharsets.UTF_8)) {
                ...
                c.ownerNationId = o.get("ownerNationId").getAsString();
                c.type = o.get("type").getAsString();
                c.value = o.get("value").getAsDouble();
                c.shares = o.get("shares").getAsInt();
    ...
    public synchronized List<Corporation> getCorporationsOf(String nationId) {
        ...
        for (Corporation c : corporations.values()) {
            if (c.ownerNationId.equals(nationId)) out.add(c);
        }
        return out;
    }
    ...
    public synchronized String payDividends(String nationId, String corporationId, double dividendAmount) throws IOException {
        ...
        if (dividendAmount <= 0) return "Неверная сумма дивидендов.";
    ...
    public synchronized Map<String, Object> getGlobalStockMarketStatistics() {
        ...
        double totalMarketValue = corporations.values().stream().mapToDouble(c -> c.value).sum();
        double maxCorporationValue = corporations.values().stream().mapToDouble(c -> c.value).max().orElse(0);
        long totalShares = corporations.values().stream().mapToLong(c -> c.shares).sum();
