...
public class AdvancedWarSystem {
    ...
    private double calculateWarCost(War war) {
        ...
        double attackerStrength = militaryService.getMilitaryStrength(war.attackerId);
        double defenderStrength = militaryService.getMilitaryStrength(war.defenderId);
    ...
    private BattleResult simulateBattle(War war, Front front) {
        ...
        double attackerTechBonus = plugin.getTechnologyTreeService().getBonus(war.attackerId, "warStrength");
        double defenderTechBonus = plugin.getTechnologyTreeService().getBonus(war.defenderId, "warStrength");
    ...
    private double getWarModBonus(String nationId) {
        ...
        if (plugin.getModIntegrationService().hasIndustrialMods()) {
            bonus += 0.03; // +3% for industrial support
        }
        
        // Check for logistics mods (supply)
        if (plugin.getModIntegrationService().hasLogisticsMods()) {
    ...
    private void updateWarStatus(War war) {
        ...
        Nation defender = nationManager.getNationById(war.defenderId);
        if (defender != null) {
            if (avgProgress >= 90 || war.territoriesCaptured >= defender.getClaimedChunkKeys().size() * 0.5) {
    ...
    public synchronized Map<String, Object> getWarStatistics(String nationId) {
        ...
        for (War war : wars) {
            ...
            if (war.status == WarStatus.ENDED) {
                double progress = war.fronts.values().stream()
                    .mapToDouble(f -> f.attackerProgress)
                    .average()
                    .orElse(0);
                
                if (isAttacker && progress >= 70) victories++;
                else if (!isAttacker && progress <= 30) victories++;
                else defeats++;
            }
        }
    ...
    private void announceWarDeclaration(War war) {
        ...
        Bukkit.getScheduler().runTask(plugin, () -> {
            ...
            for (org.bukkit.entity.Player player : Bukkit.getOnlinePlayers()) {
                ...
                plugin.getVisualEffectsService().sendActionBar(player, 
            ...
            for (UUID citizenId : attacker.getCitizens()) {
                ...
                if (citizen != null && citizen.isOnline()) {
                    ...
                    citizen.sendTitle(
                        ...
                        10, 100, 20);
                    plugin.getVisualEffectsService().sendActionBar(citizen,
