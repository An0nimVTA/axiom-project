...
public class InfrastructureMaintenanceService {
    ...
    public synchronized String allocateMaintenance(String nationId, double budget) {
        ...
        if (budget <= 0) return "Бюджет должен быть больше 0.";
        ...
        data.lastMaintenance = System.currentTimeMillis();
    ...
    private synchronized void processMaintenance() {
        for (Map.Entry<String, InfrastructureData> e : nationInfrastructure.entrySet()) {
            ...
            if ((data.roadCondition < 30 || data.buildingCondition < 30) && plugin.getHappinessService() != null) {
