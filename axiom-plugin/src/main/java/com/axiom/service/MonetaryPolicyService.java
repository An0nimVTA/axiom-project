...
public class MonetaryPolicyService {
    ...
    public synchronized String setInterestRate(String nationId, double rate) {
        if (nationId == null || nationId.trim().isEmpty() || !Double.isFinite(rate)) {
            return "Некорректные данные.";
        }
        MonetaryPolicy policy = policies.computeIfAbsent(nationId, k -> defaultPolicy());
        policy.interestRate = Math.max(0, Math.min(20, rate));
        return "Ставка процента установлена: " + policy.interestRate + "%";
    }

    public synchronized String setReserveRequirement(String nationId, double requirement) {
        if (nationId == null || nationId.trim().isEmpty() || !Double.isFinite(requirement)) {
            return "Некорректные данные.";
        }
        MonetaryPolicy policy = policies.computeIfAbsent(nationId, k -> defaultPolicy());
        policy.reserveRequirement = Math.max(0, Math.min(100, requirement));
        return "Требование резерва установлено: " + policy.reserveRequirement + "%";
    }

    public synchronized String enableQE(String nationId, double multiplier) {
        if (nationId == null || nationId.trim().isEmpty() || !Double.isFinite(multiplier) || multiplier <= 0) {
            return "Некорректные данные.";
        }
        MonetaryPolicy policy = policies.computeIfAbsent(nationId, k -> defaultPolicy());
    ...
    public synchronized double getInterestRate(String nationId) {
        if (nationId == null || nationId.trim().isEmpty()) return 5.0;
    ...
    private MonetaryPolicy defaultPolicy() {
