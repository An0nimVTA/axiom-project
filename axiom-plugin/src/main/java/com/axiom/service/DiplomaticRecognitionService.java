...
public class DiplomaticRecognitionService {
    ...
    public synchronized String recognizeNation(String recognizerId, String targetId) {
        ...
        if (recognizerId == null || targetId == null) return "Неверные параметры.";
        if (recognizerId.equals(targetId)) return "Нельзя признать собственную нацию.";
        ...
        if (target == null) return "Целевая нация не найдена.";
    ...
    public synchronized boolean isRecognized(String recognizerId, String targetId) {
        if (recognizerId == null || targetId == null) return false;
