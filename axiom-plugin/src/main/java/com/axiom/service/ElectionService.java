...
public class ElectionService {
    ...
    public synchronized String startElection(String nationId, String type, long durationMinutes, List<String> candidates) throws IOException {
        ...
        if (candidates == null || candidates.isEmpty()) return "Кандидаты не указаны.";
        ...
        e.nationId = nationId;
        e.type = type;
        e.candidates = new ArrayList<>(candidates);
            ...
            for (UUID citizenId : citizens) {
                ...
                if (citizen != null && citizen.isOnline()) {
                    if (effectsService != null) {
                        effectsService.playElectionEffect(citizen, n.getName(), typeName);
                    }
                }
            }
        });
    ...
    public synchronized String vote(UUID voter, String nationId, String type, String candidate) throws IOException {
        if (nationManager == null) return "Сервис наций недоступен.";
        if (voter == null || nationId == null || type == null || candidate == null) return "Неверные параметры.";
    ...
    public synchronized String finishElection(String nationId, String type) throws IOException {
        ...
        if (results.isEmpty()) return "Нет результатов.";
