...
public class BlockadeService {
    ...
    public synchronized String establishBlockade(String blockaderId, String targetId) {
        if (blockaderId.equals(targetId)) return "Нельзя блокировать самого себя.";
        NationManager nationManager = plugin.getNationManager();
        if (nationManager == null) return "Сервис наций недоступен.";
        Nation blockader = nationManager.getNationById(blockaderId);
        Nation target = nationManager.getNationById(targetId);
        if (blockader == null || target == null) return "Нация не найдена.";
        if (blockadedNations.getOrDefault(blockaderId, Collections.emptySet()).contains(targetId)) {
            return "Блокада уже установлена.";
        }
        // Need naval power
        NavalService navalService = plugin.getNavalService();
        if (navalService == null) return "Морской сервис недоступен.";
        NavalService.NavalPower power = navalService.getNavalPower(blockaderId);
        if (power == null) return "Не удалось определить морскую мощь.";
        int ships = power.ships;
