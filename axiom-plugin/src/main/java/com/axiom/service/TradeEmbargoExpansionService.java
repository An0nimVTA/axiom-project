...
public class TradeEmbargoExpansionService {
    ...
    public synchronized String imposeEmbargo(String embargoerId, String targetId, double severity, double cost) {
        ...
        if (isBlank(embargoerId) || isBlank(targetId)) return "Неверные параметры.";
        if (embargoerId.equals(targetId)) return "Нельзя наложить эмбарго на себя.";
        if (!Double.isFinite(cost) || cost < 0) return "Некорректная стоимость.";
        if (!Double.isFinite(severity)) return "Некорректная тяжесть.";
        Nation embargoer = plugin.getNationManager().getNationById(embargoerId);
        if (embargoer == null) return "Нация не найдена.";
        if (plugin.getNationManager().getNationById(targetId) == null) return "Целевая нация не найдена.";
        ...
        double clampedSeverity = Math.max(0, Math.min(100, severity));
        embargoSeverity.put(embargoerId + "_" + targetId, clampedSeverity);
        ...
        if (target != null) {
            if (target.getHistory() != null) {
                target.getHistory().add("Торговое эмбарго от " + embargoer.getName() + " (тяжесть: " + clampedSeverity + "%)");
            }
        ...
        return "Эмбарго наложено (тяжесть: " + clampedSeverity + "%)";
    }

    public synchronized boolean isEmbargoed(String embargoerId, String targetId) {
        if (isBlank(embargoerId) || isBlank(targetId)) return false;
    ...
    public synchronized double getEmbargoSeverity(String embargoerId, String targetId) {
        if (isBlank(embargoerId) || isBlank(targetId)) return 0.0;
    ...
    public synchronized double getTradePenalty(String nationA, String nationB) {
        if (isBlank(nationA) || isBlank(nationB)) return 0.0;
