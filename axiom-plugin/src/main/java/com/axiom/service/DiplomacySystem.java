...
public class DiplomacySystem {
    ...
    private final Map<String, Long> warActiveUntil = new ConcurrentHashMap<>(); // nationId|enemyId -> ts
    ...
    private synchronized void checkExpiredWars() {
        ...
        Set<String> handledPairs = new HashSet<>();
        for (String key : expiredWars) {
            String[] parts = key.split("\\|");
            if (parts.length == 2) {
                String nationA = parts[0];
                String nationB = parts[1];
                String canonical = nationA.compareTo(nationB) <= 0 ? nationA + "|" + nationB : nationB + "|" + nationA;
                if (!handledPairs.add(canonical)) {
                    continue;
                }
                warActiveUntil.remove(nationA + "|" + nationB);
                warActiveUntil.remove(nationB + "|" + nationA);
