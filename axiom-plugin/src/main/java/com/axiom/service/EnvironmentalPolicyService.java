...
public class EnvironmentalPolicyService {
    ...
    public synchronized String setPolicy(String nationId, double pollutionLimit, double conservationLevel, boolean carbonTax, double taxRate) {
        ...
        if (pollutionLimit < 0) return "Лимит загрязнения должен быть неотрицательным.";
        ...
        policy.carbonTaxRate = carbonTax ? Math.max(0.0, taxRate) : 0.0;
        double currentPollution = plugin.getPollutionService() != null ? plugin.getPollutionService().getPollution(nationId) : 0.0;
        if (currentPollution > policy.pollutionLimit && plugin.getHappinessService() != null) {
    ...
    private synchronized void processPolicies() {
        ...
        for (Map.Entry<String, EnvironmentalPolicy> e : nationPolicies.entrySet()) {
            ...
            if (plugin.getPollutionService() == null) continue;
    ...
    public synchronized Map<String, Object> getGlobalEnvironmentalPolicyStatistics() {
        ...
        if (plugin.getPollutionService() == null) {
            stats.put("error", "Сервис загрязнения недоступен.");
            return stats;
        }
