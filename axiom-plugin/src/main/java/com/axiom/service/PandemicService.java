...
public class PandemicService {
    ...
    public synchronized void triggerPandemic(String nationId, String diseaseName, double severity) {
        ...
        if (nationId == null || nationId.trim().isEmpty() || diseaseName == null || diseaseName.trim().isEmpty()) return;
        if (!Double.isFinite(severity)) return;
        Pandemic p = new Pandemic();
        p.nationId = nationId;
        p.diseaseName = diseaseName;
        p.severity = Math.max(0, Math.min(100, severity));
        p.spreadRate = p.severity / 10.0;
    ...
    private synchronized void spreadPandemics() {
        if (nationManager == null) return;
        Map<String, Pandemic> toSpread = new HashMap<>();
                ...
                if (n.getAllies() != null) {
                    for (String allyId : n.getAllies()) {
                        if (!activePandemics.containsKey(allyId) && Math.random() < 0.3) {
                            toSpread.put(allyId, p);
                        }
                    }
                }
            }
        }
        for (Map.Entry<String, Pandemic> entry : toSpread.entrySet()) {
            Pandemic source = entry.getValue();
            triggerPandemic(entry.getKey(), source.diseaseName, source.severity * 0.8);
        }
    }
    ...
    public synchronized String reduceSeverity(String nationId, double reduction, double cost) throws IOException {
        if (nationId == null || nationId.trim().isEmpty() || !Double.isFinite(reduction) || reduction <= 0 || !Double.isFinite(cost) || cost < 0) {
            return "Некорректные данные.";
        }
        Pandemic pandemic = activePandemics.get(nationId);
        if (pandemic == null) return "Пандемии нет.";
    ...
    public synchronized Map<String, Object> getGlobalPandemicStatistics() {
        ...
        if (nationManager == null) {
            ...
            stats.put("averageSpreadRate", activePandemics.size() > 0 ? totalSpreadRate / activePandemics.size() : 0);
            stats.put("nationsWithPandemics", activePandemics.size());
            stats.put("severityDistribution", new HashMap<>());
            stats.put("topBySeverity", new ArrayList<>());
            stats.put("mostCommonDiseases", new ArrayList<>());
