...
public class AxiomCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) {
            if (sender instanceof Player) {
                plugin.openUiMainMenu((Player) sender);
                sender.sendMessage("§7Если меню не открылось, используйте §b/axiom help");
            } else {
    ...
    private void sendHelpMessage(CommandSender sender) {
        ...
        sender.sendMessage(formatHelpCommand("/axiom menu", "Открыть главное меню AXIOM."));
    ...
    private boolean handleElection(CommandSender sender, String[] args) {
        ...
        // Open GUI
        new com.axiom.app.gui.ElectionMenu(plugin, p).open();
        return true;
    }
    ...
    private boolean handleWar(CommandSender sender, String[] args) {
        ...
        if (args.length >= 2 && args[1].equalsIgnoreCase("declare")) {
            ...
            if (plugin.getAdvancedWarSystem() != null) {
                List<com.axiom.domain.service.military.AdvancedWarSystem.War> wars = plugin.getAdvancedWarSystem().getNationWars(opt.get().getId());
                if (wars.isEmpty()) {
                    sender.sendMessage("§aНет активных войн.");
                } else {
                    sender.sendMessage("§c§lАКТИВНЫЕ ВОЙНЫ: " + wars.size());
                    for (com.axiom.domain.service.military.AdvancedWarSystem.War war : wars) {
                        ...
                        sender.sendMessage("§7  Тип: §f" + war.type.name() + " §7| Статус: §f" + war.status.name()
                            + " §7| Стадия: §f" + (war.stage != null ? war.stage.name() : "UNKNOWN"));
    ...
    private boolean handleBanking(CommandSender sender, String[] args) {
        ...
        if (args.length >= 2 && args[1].equalsIgnoreCase("list")) {
            com.axiom.domain.service.industry.BankingService banking = plugin.getBankingService();
            if (banking != null) {
                List<com.axiom.domain.service.industry.BankingService.Loan> loans = banking.getActiveLoans(opt.get().getId());
                if (loans.isEmpty()) {
                    sender.sendMessage("§aНет активных кредитов.");
                } else {
                    sender.sendMessage("§bАктивные кредиты: §f" + loans.size());
                    for (com.axiom.domain.service.industry.BankingService.Loan loan : loans) {
    ...
    private boolean handleStock(CommandSender sender, String[] args) {
        ...
        com.axiom.domain.service.industry.StockMarketService stock = plugin.getStockMarketService();
        if (stock != null) {
            if (args.length >= 2 && args[1].equalsIgnoreCase("list")) {
                List<com.axiom.domain.service.industry.StockMarketService.Corporation> corps = stock.getCorporationsOf(opt.get().getId());
                sender.sendMessage("§bКорпорации вашей нации: §f" + corps.size());
                for (com.axiom.domain.service.industry.StockMarketService.Corporation corp : corps) {
                ...
                List<com.axiom.domain.service.industry.StockMarketService.Corporation> allCorps = stock.getAllCorporations();
                ...
                List<com.axiom.domain.service.industry.StockMarketService.Corporation> top = stock.getTopCorporationsByValue(10);
                sender.sendMessage("§bТоп-10 по стоимости:");
                for (int i = 0; i < Math.min(10, top.size()); i++) {
                    com.axiom.domain.service.industry.StockMarketService.Corporation corp = top.get(i);
    ...
    private boolean handleCulture(CommandSender sender, String[] args) {
        ...
        com.axiom.domain.service.politics.CultureService culture = plugin.getCultureService();
        if (culture != null) {
            // Get culture stats
    ...
    private boolean handleAdvancedFeatures(CommandSender sender, String[] args) {
        ...
        Player p = (Player) sender;
        new com.axiom.app.gui.AdvancedFeaturesMenu(plugin, p).open();
        return true;
    }
    ...
    private boolean handleMapVisualization(CommandSender sender, String[] args) {
        ...
        Player p = (Player) sender;
        new com.axiom.app.gui.MapBoundaryVisualizationMenu(plugin, p).open();
        return true;
    }
    ...
    private boolean handleEconomicIndicators(CommandSender sender, String[] args) {
        ...
        Player p = (Player) sender;
        new com.axiom.app.gui.EconomicIndicatorsMenu(plugin, p).open();
        return true;
    }
}
