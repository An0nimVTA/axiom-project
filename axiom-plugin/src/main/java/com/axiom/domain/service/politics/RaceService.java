...
public class RaceService {
    ...
    public synchronized String setPlayerRace(UUID playerId, String raceId) {
        if (playerId == null) return "Неверный идентификатор игрока.";
        if (isBlank(raceId)) return "Неверный идентификатор расы.";
    ...
    public synchronized double getBonus(UUID playerId, String bonusType) {
        if (playerId == null || isBlank(bonusType)) return 1.0;
    ...
    private void savePlayerRace(UUID playerId, String raceId) {
        if (playerId == null || isBlank(raceId)) return;
        File f = new File(racesDir, "players.json");
    ...
    private void saveRace(Race race) {
        if (race == null || isBlank(race.id)) return;
    ...
    public synchronized double getNationRacialDiversity(String nationId) {
        if (isBlank(nationId) || plugin.getNationManager() == null) return 1.0;
        ...
        if (citizens == null || citizens.isEmpty()) return 1.0;
    ...
    public synchronized List<UUID> getPlayersOfRace(String raceId) {
        ...
        for (Map.Entry<UUID, String> entry : playerRaces.entrySet()) {
            if (Objects.equals(entry.getValue(), raceId)) {
    ...
    public synchronized Map<String, Object> getGlobalRaceStatistics() {
        ...
        if (plugin.getNationManager() == null || plugin.getNationManager().getAll() == null) {
            stats.put("averageRacialDiversityPerNation", 0);
        } else {
            for (com.axiom.domain.model.Nation n : plugin.getNationManager().getAll()) {
                if (n == null) continue;
                Collection<UUID> citizens = n.getCitizens();
                if (citizens == null || citizens.isEmpty()) continue;
                Set<String> uniqueRaces = new HashSet<>();
                for (UUID citizenId : citizens) {
                    String raceId = getPlayerRace(citizenId);
                    uniqueRaces.add(raceId);
