...
public class DisasterService {
    ...
    public synchronized void triggerDisaster(String nationId, String type, double severity, long durationMinutes) {
        ...
        if (nationId == null || type == null || type.trim().isEmpty()) return;
        if (durationMinutes <= 0) return;
        ActiveDisaster d = new ActiveDisaster();
        d.nationId = nationId;
        d.type = type;
        d.severity = Math.max(0, Math.min(100, severity));
            ...
            for (UUID citizen : citizens) {
                ...
                if (p != null && p.isOnline()) {
                    ...
                    if (effectsService != null) {
                        effectsService.sendActionBar(p, msg + " §7(Уровень: " + String.format("%.0f", d.severity) + "%)");
                    }
    ...
    public synchronized String endDisaster(String nationId, double cost) throws IOException {
        ...
        ActiveDisaster d = activeDisasters.get(nationId);
        ...
        activeDisasters.remove(nationId);
        ...
        org.bukkit.Bukkit.getScheduler().runTask(plugin, () -> {
            ...
            for (UUID citizen : citizens) {
                ...
                if (p != null && p.isOnline()) {
                    if (effectsService != null) {
                        effectsService.sendActionBar(p, msg);
                    }
                    org.bukkit.Location loc = p.getLocation();
    ...
    public synchronized double getDisasterRisk(String nationId) {
        ...
        if (nationManager == null) return risk;
        Nation n = nationManager.getNationById(nationId);
