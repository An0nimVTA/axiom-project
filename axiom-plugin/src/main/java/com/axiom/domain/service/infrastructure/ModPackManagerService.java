...
public class ModPackManagerService {
    ...
    public static class ModPack {
        ...
        public Map<String, Object> configuration; // Specific settings for this modpack
        public boolean enabled;
        public int recommendedPlayers; // Recommended player count for this modpack
        public String compatibilityLevel; // "balanced", "hardcore", "casual"
        public List<ModIntegrationRule> integrationRules;
    ...
    public static class ModIntegrationRule {
        ...
        public String conversionType; // "direct", "recipe", "effect", "stat_boost"
        public String conversionItem;
    ...
    private void initializeConfig() {
        ...
        modPackConfig = new YamlConfiguration();
    ...
    private void loadModPacks() {
        if (modPackConfig != null && modPackConfig.contains("modpacks")) {
            if (modPackConfig.getConfigurationSection("modpacks") == null) {
                return;
            }
            for (String modPackId : modPackConfig.getConfigurationSection("modpacks").getKeys(false)) {
                ...
                if (modPackConfig.contains(path + ".integrationRules") &&
                    modPackConfig.getConfigurationSection(path + ".integrationRules") != null) {
    ...
    public boolean enableModPackForNation(String nationId, String modPackId) {
        if (nationId == null || nationId.trim().isEmpty() || modPackId == null || modPackId.trim().isEmpty()) {
            return false;
        }
    ...
    private void activateIntegrationRules(String nationId, ModPack modPack) {
        if (recipeIntegrationService == null || modPack == null) return;
    ...
    private void enableCrossModRecipe(ModIntegrationRule rule) {
        ...
        if (recipeIntegrationService == null || rule == null) return;
    ...
    private boolean areRequiredModsAvailable(ModPack modPack) {
        if (plugin.getModIntegrationService() == null || modPack == null) {
            return false;
        }
    ...
    public String getActiveModPackForNation(String nationId) {
        if (nationId == null || nationId.trim().isEmpty()) return null;
    ...
    public void applyNationModPackBonuses(String nationId) {
        if (nationId == null || nationId.trim().isEmpty()) return;
    ...
    public void saveConfig() {
        try {
            if (modPackConfig == null || modPackFile == null) return;
    ...
    public boolean removeModPack(String modPackId) {
        if (modPackId == null || modPackId.trim().isEmpty()) return false;
    ...
    private void loadModPacksFromConfig() {
        ...
        if (externalConfig.contains("definitions") && externalConfig.getConfigurationSection("definitions") != null) {
            for (String modPackId : externalConfig.getConfigurationSection("definitions").getKeys(false)) {
                ...
                if (externalConfig.contains(path + ".integrationRules") &&
                    externalConfig.getConfigurationSection(path + ".integrationRules") != null) {
