...
public class ConquestService {
    ...
    public synchronized void updateConquestProgress(String attackerId, String defenderId, int progress) {
        ...
        if (relationService != null && !relationService.isAtWar(attackerId, defenderId)) {
            return;
        }
        AdvancedWarSystem warSystem = plugin.getAdvancedWarSystem();
        if (warSystem != null && !warSystem.isNationInActiveWar(attackerId)) {
            return;
        }
    ...
    private void completeConquest(String attackerId, String defenderId) {
        ...
        AdvancedWarSystem warSystem = plugin.getAdvancedWarSystem();
        if (warSystem != null && transferred > 0) {
            warSystem.recordTerritoryCapture(attackerId, defenderId, transferred);
        }
        ...
        MapBoundaryService mapService = plugin.getMapBoundaryService();
        if (mapService != null) {
