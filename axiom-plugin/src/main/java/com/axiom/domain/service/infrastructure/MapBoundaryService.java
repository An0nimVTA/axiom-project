...
public class MapBoundaryService {
    ...
    public MapBoundaryService(AXIOM plugin,
                              NationManager nationManager,
                              CityGrowthEngine cityEngine,
                              ModIntegrationService modIntegration,
    ...
    public synchronized void updateAllBoundaries() {
        ...
        for (Nation nation : nationManager.getAll()) {
            ...
            Set<String> chunks = toChunkKeys(nationId);
    ...
    private Set<String> toChunkKeys(String nationId) {
        return resolveNationChunkKeys(nationManager, territoryService, nationId);
    }

    static Set<String> resolveNationChunkKeys(NationManager nationManager,
                                              TerritoryService territoryService,
                                              String nationId) {
        if (nationId == null) {
            return new HashSet<>();
        ...
        if (territoryService == null) {
            ...
            return new HashSet<>(nation != null && nation.getClaimedChunkKeys() != null
                ? nation.getClaimedChunkKeys()
                : Collections.emptySet());
        }
        Set<com.axiom.domain.model.ChunkPos> claims = territoryService.getNationClaims(nationId);
        Set<String> keys = new HashSet<>();
        for (com.axiom.domain.model.ChunkPos pos : claims) {
