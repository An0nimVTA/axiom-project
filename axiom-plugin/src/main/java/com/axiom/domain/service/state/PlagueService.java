...
public class PlagueService {
    ...
    private void applyPlagueEffects(Plague p) {
        // Reduce population
        com.axiom.domain.model.City city = plugin.getCityGrowthEngine().getCitiesOf(p.nationId).stream()
    ...
    public synchronized Map<String, Object> getGlobalPlagueStatistics() {
        ...
        Map<String, Double> totalMortalityByNation = new HashMap<>();
        Map<String, Integer> plagueCountByNation = new HashMap<>();
        ...
        for (Plague p : activePlagues.values()) {
            ...
            totalMortalityByNation.put(p.nationId,
                totalMortalityByNation.getOrDefault(p.nationId, 0.0) + p.mortalityRate);
            plagueCountByNation.put(p.nationId,
                plagueCountByNation.getOrDefault(p.nationId, 0) + 1);
        }
        
        for (Map.Entry<String, Double> entry : totalMortalityByNation.entrySet()) {
            int count = plagueCountByNation.getOrDefault(entry.getKey(), 1);
            averageMortalityByNation.put(entry.getKey(), entry.getValue() / count);
        }
