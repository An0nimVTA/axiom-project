...
public class PartisanService {
    ...
    public synchronized void establishPartisans(String territoryKey, String originalNationId, String occupierNationId) {
        if (territoryKey == null || territoryKey.trim().isEmpty() || originalNationId == null || originalNationId.trim().isEmpty()
            || occupierNationId == null || occupierNationId.trim().isEmpty()) {
            return;
        }
        ...
        saveGroup(pg);
        Nation occupier = plugin.getNationManager() != null ? plugin.getNationManager().getNationById(occupierNationId) : null;
        if (occupier != null) {
    ...
    private synchronized void partisanActions() {
    ...
    private void sabotageOccupier(PartisanGroup pg) {
        Nation occupier = plugin.getNationManager() != null ? plugin.getNationManager().getNationById(pg.occupierNationId) : null;
    ...
    public synchronized Map<String, Object> getPartisanStatistics(String nationId) {
        ...
        if (nationId == null || nationId.trim().isEmpty()) {
            stats.put("occupyingTerritories", 0);
            stats.put("createdGroups", 0);
            stats.put("groupsList", new ArrayList<>());
            stats.put("averageActivityLevel", 0.0);
            stats.put("rating", "НЕТ УГРОЗ");
            return stats;
        }
        ...
        for (PartisanGroup pg : groups.values()) {
            if (nationId.equals(pg.occupierNationId)) {
                occupyingGroups.add(pg);
            }
            if (nationId.equals(pg.originalNationId)) {
    ...
    public synchronized String suppressPartisans(String territoryKey, String nationId, double cost) throws IOException {
        if (territoryKey == null || territoryKey.trim().isEmpty() || nationId == null || nationId.trim().isEmpty()
    ...
    public synchronized List<PartisanGroup> getOccupyingGroups(String nationId) {
        if (nationId == null || nationId.trim().isEmpty()) return new ArrayList<>();
