...
public class ResourceDiscoveryService {
    ...
    private void generateDiscoveries() {
        ...
        for (Nation n : nationManager.getAll()) {
            ...
            if (claimedChunks == null || claimedChunks.isEmpty()) continue;
            if (Math.random() < 0.1) { // 10% chance per nation
                String resource = pickDiscoveryResource();
                if (resource == null) continue;
                List<String> chunkKeys = new ArrayList<>(claimedChunks);
                String chunkKey = chunkKeys.get((int)(Math.random() * chunkKeys.size()));
    ...
    private String pickDiscoveryResource() {
        ResourceCatalogService catalog = plugin.getResourceCatalogService();
        Set<String> resources = catalog != null ? catalog.getDiscoveryResources() : Collections.emptySet();
        if (!resources.isEmpty()) {
            List<String> list = new ArrayList<>(resources);
            return list.get((int) (Math.random() * list.size()));
        }
        String[] fallback = {"oil", "gold", "iron", "coal", "rare_metals"};
