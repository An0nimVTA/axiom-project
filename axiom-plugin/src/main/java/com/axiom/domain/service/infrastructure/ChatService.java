...
public class ChatService {
    ...
    public synchronized boolean sendNationChat(UUID senderId, String message) {
        ...
        String nationId = playerDataManager.getNation(senderId);
        if (nationId == null) return false;
        Player sender = Bukkit.getPlayer(senderId);
        if (sender == null) return false;
        String format = "§b[N] §f" + sender.getName() + ": §7" + message;
        for (Player p : Bukkit.getOnlinePlayers()) {
            String pNation = playerDataManager.getNation(p.getUniqueId());
    ...
    public synchronized boolean sendDiplomaticChat(UUID senderId, String message) {
        ...
        Player sender = Bukkit.getPlayer(senderId);
        if (sender == null) return false;
        com.axiom.domain.model.Nation senderNation = nationManager.getNationById(nationId);
        if (senderNation == null) return false;
        String format = "§b[D] §f" + senderNation.getName() + " (" + sender.getName() + "): §7" + message;
        Set<String> recipients = new HashSet<>();
        if (senderNation.getAllies() != null) {
            recipients.addAll(senderNation.getAllies());
        }
        Set<String> pending = senderNation.getPendingAlliance();
        if (pending != null) {
            recipients.addAll(pending.stream()
    ...
    public synchronized Map<String, Object> getChatStatistics(UUID playerId) {
        ...
        if (channel.equals("nation") && nationId != null) {
            ...
            com.axiom.domain.model.Nation n = nationManager != null ? nationManager.getNationById(nationId) : null;
            if (n != null) {
                Set<String> allies = n.getAllies() != null ? new HashSet<>(n.getAllies()) : new HashSet<>();
                for (Player p : Bukkit.getOnlinePlayers()) {
                    String pNation = playerDataManager.getNation(p.getUniqueId());
        ...
        if (nationId != null) {
            ...
            com.axiom.domain.model.Nation n = nationManager != null ? nationManager.getNationById(nationId) : null;
            if (n != null && n.getAllies() != null && !n.getAllies().isEmpty()) {
    ...
    public synchronized List<UUID> getNationChatParticipants(String nationId) {
        ...
        PlayerDataManager playerDataManager = plugin.getPlayerDataManager();
        if (playerDataManager == null) return participants;
        for (Player p : Bukkit.getOnlinePlayers()) {
            String pNation = playerDataManager.getNation(p.getUniqueId());
            if (pNation != null && pNation.equals(nationId)) {
