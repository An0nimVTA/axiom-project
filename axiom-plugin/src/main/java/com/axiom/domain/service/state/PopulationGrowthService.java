...
public class PopulationGrowthService {
    ...
    private synchronized void processGrowth() {
        ...
        for (Nation n : nationManager.getAll()) {
            PopulationData data = nationPopulations.computeIfAbsent(n.getId(), k -> {
                ...
                d.totalPopulation = (n.getCitizens() != null ? n.getCitizens().size() : 0) * 10.0; // 10 NPCs per citizen
            ...
            int citizenCount = n.getCitizens() != null ? n.getCitizens().size() : 0;
            data.totalPopulation = Math.max(citizenCount, data.totalPopulation + growth);
    ...
    public synchronized Map<String, Object> getPopulationStatistics(String nationId) {
        ...
        if (data == null || n == null) {
            int citizens = n != null && n.getCitizens() != null ? n.getCitizens().size() : 0;
            stats.put("totalPopulation", citizens * 10.0);
        ...
        int citizens = n.getCitizens() != null ? n.getCitizens().size() : 0;
        stats.put("citizens", citizens);
        stats.put("npcs", data.totalPopulation - citizens);
    ...
    public synchronized String applyMigration(String nationId, double migrationAmount) {
        if (nationId == null || nationId.trim().isEmpty() || !Double.isFinite(migrationAmount)) {
    ...
    public synchronized Map<String, Object> getGlobalPopulationStatistics() {
        ...
        if (nationManager == null) {
            ...
            stats.put("nationsWithPopulation", 0);
            stats.put("totalPopulation", 0.0);
            stats.put("maxPopulation", 0.0);
            stats.put("averagePopulation", 0.0);
            stats.put("averageGrowthRate", 0.0);
            stats.put("averageBirthRate", 0.0);
            stats.put("averageDeathRate", 0.0);
            stats.put("topByPopulation", new ArrayList<>());
            return stats;
        }
        for (Nation n : nationManager.getAll()) {
            PopulationData data = nationPopulations.get(n.getId());
            double population = data != null ? data.totalPopulation :
                ((n.getCitizens() != null ? n.getCitizens().size() : 0) * 10.0);
