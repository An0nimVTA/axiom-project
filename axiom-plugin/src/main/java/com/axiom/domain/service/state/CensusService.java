...
public class CensusService {
    ...
    private void conductCensus() {
        CityGrowthEngine cityGrowthEngine = plugin.getCityGrowthEngine();
        for (Nation n : nationManager.getAll()) {
            ...
            data.activePlayers = (int) n.getCitizens().stream()
                ...
                .count();
            data.cities = cityGrowthEngine != null ? cityGrowthEngine.getCitiesOf(n.getId()).size() : 0;
    ...
    private void loadAll() {
        ...
        for (File f : files) {
            try (Reader r = new InputStreamReader(Files.newInputStream(f.toPath()), StandardCharsets.UTF_8)) {
                ...
                CensusData data = new CensusData();
                data.totalPopulation = o.has("totalPopulation") ? o.get("totalPopulation").getAsInt() : 0;
                data.activePlayers = o.has("activePlayers") ? o.get("activePlayers").getAsInt() : 0;
                data.cities = o.has("cities") ? o.get("cities").getAsInt() : 0;
                data.conductedAt = o.has("conductedAt") ? o.get("conductedAt").getAsLong() : 0L;
    ...
    public synchronized String conductCensusNow(String nationId) {
        ...
        data.activePlayers = (int) n.getCitizens().stream()
            .filter(uuid -> plugin.getServer().getPlayer(uuid) != null && plugin.getServer().getPlayer(uuid).isOnline())
            .count();
        data.cities = cityGrowthEngine != null ? cityGrowthEngine.getCitiesOf(nationId).size() : 0;
        data.conductedAt = System.currentTimeMillis();
        ...
        for (Nation n : nationManager.getAll()) {
            ...
            if (data != null) {
                ...
                int currentActive = (int) n.getCitizens().stream()
                    .filter(uuid -> plugin.getServer().getPlayer(uuid) != null && plugin.getServer().getPlayer(uuid).isOnline())
                    .count();
                int currentCities = cityGrowthEngine != null ? cityGrowthEngine.getCitiesOf(nationId).size() : 0;
