...
public class CeasefireService {
    ...
    public synchronized String createCeasefire(String nationA, String nationB, int durationHours, boolean permanent, String terms) {
        ...
        if (!relationService.isAtWar(nationA, nationB)) {
        ...
        try {
            ...
            long durationMs = permanent ? 0 : durationHours * 60L * 60_000L;
            relationService.setStatus(nationA, nationB, DiplomacyRelationService.RelationStatus.CEASEFIRE,
                durationMs, "ceasefire");
            AdvancedWarSystem warSystem = plugin.getAdvancedWarSystem();
            if (warSystem != null) {
                warSystem.enterCeasefire(nationA, nationB, durationMs);
            }
