...
public class AdvancedWarSystem {
    ...
    private static final long PREPARATION_DURATION_MS = 10L * 60L * 1000L;
    ...
    public static class War {
        ...
        public WarStage stage;                         // Current stage
        public long startTime;                         // When war started
        public long endTime;                          // When war ended (0 if active)
        public long preparationEndsAt;                 // When preparation ends
        public long ceasefireEndsAt;                   // When ceasefire ends
    ...
    public AdvancedWarSystem(AXIOM plugin, NationManager nationManager, DiplomacySystem diplomacySystem,
        ...
        this.relationService = plugin.getDiplomacyRelationService();
    ...
    public synchronized String declareAdvancedWar(Nation attacker, Nation defender, WarType type, List<String> goals) throws IOException {
        ...
        } else if (diplomacySystem.isAtWar(attacker.getId(), defender.getId())) {
            return "Война уже активна.";
        }

        long now = System.currentTimeMillis();
        War war = initWarRecord(attacker.getId(), defender.getId(), type, goals, now);
        ...
        nationWars.computeIfAbsent(attacker.getId(), k -> new HashSet<>()).add(war.id);
        nationWars.computeIfAbsent(defender.getId(), k -> new HashSet<>()).add(war.id);

        if (relationService != null) {
            relationService.setStatus(attacker.getId(), defender.getId(),
                DiplomacyRelationService.RelationStatus.WAR, Long.MAX_VALUE, "advancedWar");
        }
    ...
    private War initWarRecord(String attackerId, String defenderId, WarType type, List<String> goals, long now) {
        String warId = UUID.randomUUID().toString().substring(0, 8);
        War war = new War();
        war.id = warId;
        war.attackerId = attackerId;
        war.defenderId = defenderId;
        war.type = type != null ? type : WarType.TERRITORIAL;
        war.status = WarStatus.DECLARED;
        war.stage = WarStage.PREPARATION;
        war.startTime = now;
        war.endTime = 0;
        war.preparationEndsAt = now + PREPARATION_DURATION_MS;
        war.ceasefireEndsAt = 0;
        war.battlesFought = 0;
        war.attackerWins = 0;
        war.defenderWins = 0;
        ...
        war.attackerGoals = goals != null ? new ArrayList<>(goals) : new ArrayList<>();
    ...
    /**
     * Register a basic war declared via DiplomacySystem.
     */
    public synchronized void registerDiplomaticWar(String attackerId, String defenderId) {
        if (attackerId == null || defenderId == null) return;
        if (getActiveWar(attackerId, defenderId) != null) return;
        Nation attacker = nationManager.getNationById(attackerId);
        Nation defender = nationManager.getNationById(defenderId);
        if (attacker == null || defender == null) return;
        long now = System.currentTimeMillis();
        War war = initWarRecord(attackerId, defenderId, WarType.TERRITORIAL,
            Arrays.asList("Захватить территории", "Ослабить противника"), now);
        initializeFronts(war);
        activeWars.put(war.id, war);
        nationWars.computeIfAbsent(attackerId, k -> new HashSet<>()).add(war.id);
        nationWars.computeIfAbsent(defenderId, k -> new HashSet<>()).add(war.id);
        saveWar(war);
    }

    /**
     * Enter ceasefire stage for an active war.
     */
    public synchronized void enterCeasefire(String nationA, String nationB, long durationMs) {
        War war = getActiveWar(nationA, nationB);
        if (war == null) return;
        war.stage = WarStage.CEASEFIRE;
        war.status = WarStatus.NEAR_PEACE;
        if (durationMs > 0) {
            war.ceasefireEndsAt = System.currentTimeMillis() + durationMs;
        } else {
            war.ceasefireEndsAt = Long.MAX_VALUE;
        }
        saveWar(war);
    }

    /**
     * End war by nation pair (peace treaty).
     */
    public synchronized void endWarByNations(String nationA, String nationB) {
        War war = getActiveWar(nationA, nationB);
        if (war == null) return;
        war.stage = WarStage.ENDED;
    ...
    /**
     * Record territory capture from external conquest logic.
     */
    public synchronized void recordTerritoryCapture(String attackerId, String defenderId, int count) {
        if (count <= 0) return;
        War war = getActiveWar(attackerId, defenderId);
        if (war == null) return;
        war.territoriesCaptured += count;
        war.territoriesLost += count;
        double avgProgress = getAverageFrontProgress(war);
        checkVictoryConditions(war, avgProgress);
        saveWar(war);
    }

    private double getAverageFrontProgress(War war) {
        if (war.fronts == null || war.fronts.isEmpty()) return 0.0;
        return war.fronts.values().stream()
    ...
    private synchronized void processWars() {
        ...
        for (War war : activeWars.values()) {
            if (war.status == WarStatus.ENDED || war.stage == WarStage.ENDED) {
                warsToEnd.add(war.id);
                continue;
            }

            if (relationService != null) {
                DiplomacyRelationService.RelationStatus relStatus = relationService.getStatus(war.attackerId, war.defenderId);
                if (relStatus == DiplomacyRelationService.RelationStatus.CEASEFIRE) {
                    if (war.stage != WarStage.CEASEFIRE) {
                        war.stage = WarStage.CEASEFIRE;
                        war.status = WarStatus.NEAR_PEACE;
                        if (war.ceasefireEndsAt == 0) {
                            war.ceasefireEndsAt = Long.MAX_VALUE;
                        }
                    }
                } else if (relStatus != DiplomacyRelationService.RelationStatus.WAR) {
                    warsToEnd.add(war.id);
                    continue;
                }
            }

            if (war.stage == WarStage.PREPARATION) {
                if (now >= war.preparationEndsAt) {
                    war.stage = WarStage.ACTIVE;
            ...
            if (war.stage == WarStage.CEASEFIRE) {
                if (war.ceasefireEndsAt > 0 && war.ceasefireEndsAt != Long.MAX_VALUE && now >= war.ceasefireEndsAt) {
                    try {
                        diplomacySystem.declarePeace(war.attackerId, war.defenderId);
                    } catch (Exception ignored) {}
                    warsToEnd.add(war.id);
                } else if (relationService != null) {
                    DiplomacyRelationService.RelationStatus relStatus = relationService.getStatus(war.attackerId, war.defenderId);
                    if (relStatus != DiplomacyRelationService.RelationStatus.CEASEFIRE) {
                        warsToEnd.add(war.id);
                    }
    ...
    private synchronized void captureTerritories(War war, int count) {
        ...
        for (String chunk : defenderChunks) {
            ...
            syncTerritoryTransfer(chunk, attacker.getId(), defender.getId());
            captured++;
        }
        ...
        try {
            ...
            com.axiom.domain.service.infrastructure.MapBoundaryService mapService = plugin.getMapBoundaryService();
            if (mapService != null) {
    ...
    private void checkVictoryConditions(War war, double avgProgress) {
        if (war.stage != WarStage.ACTIVE) return;
        Nation defender = nationManager.getNationById(war.defenderId);
        if (defender != null) {
    ...
    private synchronized void updateFronts() {
        for (War war : activeWars.values()) {
            if (war.status == WarStatus.ENDED || war.stage != WarStage.ACTIVE) continue;
    ...
    /**
     * Check if nation is in an active (non-ceasefire) war stage.
     */
    public synchronized boolean isNationInActiveWar(String nationId) {
        if (nationId == null) return false;
        for (War war : getNationWars(nationId)) {
            if (war.stage == WarStage.ACTIVE) {
                return true;
            }
        }
        return false;
    ...
    private void syncTerritoryTransfer(String chunkKey, String attackerId, String defenderId) {
        if (chunkKey == null) {
            return;
        }
        ...
        try {
            ...
            if (defenderId != null) {
                territoryService.unclaim(defenderId, world, x, z);
            }
            if (attackerId != null) {
                territoryService.claim(attackerId, world, x, z);
            }
    ...
    private JsonObject serializeWar(War war) {
        ...
        o.addProperty("stage", war.stage != null ? war.stage.name() : WarStage.ACTIVE.name());
        o.addProperty("startTime", war.startTime);
        o.addProperty("endTime", war.endTime);
        o.addProperty("preparationEndsAt", war.preparationEndsAt);
        o.addProperty("ceasefireEndsAt", war.ceasefireEndsAt);
    ...
    private War deserializeWar(JsonObject o) {
        ...
        if (o.has("stage")) {
            war.stage = WarStage.valueOf(o.get("stage").getAsString());
        } else {
            if (war.status == WarStatus.DECLARED) {
                war.stage = WarStage.PREPARATION;
            } else if (war.status == WarStatus.ENDED) {
                war.stage = WarStage.ENDED;
            } else {
                war.stage = WarStage.ACTIVE;
            }
        ...
        war.preparationEndsAt = o.has("preparationEndsAt") ? o.get("preparationEndsAt").getAsLong() : (war.startTime + PREPARATION_DURATION_MS);
        war.ceasefireEndsAt = o.has("ceasefireEndsAt") ? o.get("ceasefireEndsAt").getAsLong() : 0;
