...
public class CultureShockService {
    ...
    public synchronized void applyShock(UUID playerId, String oldNationId, String newNationId) {
        ...
        if (plugin.getNationManager() == null || plugin.getCultureService() == null) return;
    ...
    private synchronized void processShock() {
        // Gradually reduce shock over time
        for (Map.Entry<String, Map<String, Double>> playerEntry : playerShock.entrySet()) {
            boolean changed = false;
            for (Map.Entry<String, Double> entry : playerEntry.getValue().entrySet()) {
                double newShock = Math.max(0, entry.getValue() - 0.5);
                if (newShock != entry.getValue()) {
                    entry.setValue(newShock);
                    changed = true;
                }
            }
            if (changed) {
                saveShock(playerEntry.getKey(), playerEntry.getValue());
            }
        }
    }

    public synchronized double getShock(UUID playerId, String nationId) {
        if (playerId == null || nationId == null) return 0.0;
