...
public class EnergyService {
    ...
    public synchronized String buildPowerPlant(String nationId, String source, double cost, double production) {
        ...
        if (nationId == null || source == null || source.trim().isEmpty()) return "Неверные параметры.";
        if (cost <= 0 || production <= 0) return "Стоимость и производство должны быть больше 0.";
    ...
    private synchronized void processEnergy() {
        for (Map.Entry<String, EnergyData> e : nationEnergy.entrySet()) {
            ...
            if (ed.storage < 0) {
                ...
                if (plugin.getNationManager() != null && plugin.getHappinessService() != null) {
