...
public class Nation {
    ...
    // --- Methods for Dynmap Integration ---

    /**
     * Generates a deterministic color for the nation based on its ID.
     * @return A Color object.
     */
    public Color getColor() {
        return Color.getHSBColor((float) (id.hashCode() % 360) / 360.0f, 0.8f, 0.9f);
    }

    /**
     * Gets the name of the world the nation is in, based on its capital chunk.
     * @return The world name, or null if the capital is not set.
     */
    public String getWorldName() {
        if (capitalChunkStr == null || !capitalChunkStr.contains(":")) {
            return null;
        }
        return capitalChunkStr.split(":")[0];
    }

    /**
     * Converts the set of claimed chunk keys into a set of actual Chunk objects.
     * @return A Set of Chunk objects.
     */
    public Set<Chunk> getClaimedChunks() {
        ...
        return claimedChunkKeys.stream()
                .map(key -> {
                    try {
