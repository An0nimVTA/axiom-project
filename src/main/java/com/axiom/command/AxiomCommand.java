...
public class AxiomCommand implements CommandExecutor {
        ...
        switch (sub) {
            ...
            case "nation":
                if (args.length >= 2 && args[1].equalsIgnoreCase("create")) {
                    return new CreateNationCommand()
                            .onCommand(sender, command, label, args);
                }
                if (sender instanceof Player) {
                    plugin.getNationMainMenu().open((Player) sender);
                    return true;
                }
            ...
            case "city":
                ...
                plugin.getCitiesMenu().open((Player) sender);
                return true;
            case "economy":
                return handleEconomy(sender, command, label, args);
            case "claim":
                // Pass only "claim" as args[0] to ClaimCommand
                return new ClaimCommand().onCommand(sender, command, label, new String[]{"claim"});
            case "unclaim":
                // Pass only "unclaim" as args[0] to UnclaimCommand
                return new UnclaimCommand().onCommand(sender, command, label, new String[]{"unclaim"});
            case "pvp":
    ...
    private boolean handleReligion(CommandSender sender, String[] args) {
        ...
        // open menu fallback
        ClientUiService uiService = ServiceLocator.get(ClientUiService.class);
        if (uiService.openReligionUi((Player) sender)) {
            return true;
        }
        plugin.getReligionMain().open((Player) sender);
        return true;
    }
