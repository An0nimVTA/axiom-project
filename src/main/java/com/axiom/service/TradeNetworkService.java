...
public class TradeNetworkService {
    ...
    public synchronized String joinNetwork(String networkId, String nationId) {
        ...
        network.tradeBonus += 0.02; // +2% per member (max 5 members = +10% total)
    ...
    private void loadAll() {
        ...
        for (File f : files) {
            try (Reader r = new InputStreamReader(Files.newInputStream(f.toPath()), StandardCharsets.UTF_8)) {
                ...
                TradeNetwork network = new TradeNetwork();
                network.id = o.get("id").getAsString();
                network.name = o.get("name").getAsString();
                network.tradeBonus = o.get("tradeBonus").getAsDouble();
                network.establishedAt = o.get("establishedAt").getAsLong();
    ...
    public synchronized Map<String, Object> getTradeNetworkStatistics(String nationId) {
        ...
        for (TradeNetwork network : networks.values()) {
            if (network.active && network.memberNations.contains(nationId)) {
                networksJoined++;
