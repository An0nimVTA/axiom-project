...
public class ResourceStockpileService {
    ...
    private void processStockpiles() {
        ...
        for (Map.Entry<String, Stockpile> e : nationStockpiles.entrySet()) {
            ...
            for (String resource : new String[]{"food", "wood"}) {
                double current = stockpile.resources.getOrDefault(resource, 0.0);
                if (current > 0) {
                    double decay = current * 0.001; // 0.1% decay per 10 minutes
    ...
    private void loadAll() {
        ...
        for (File f : files) {
            try (Reader r = new InputStreamReader(Files.newInputStream(f.toPath()), StandardCharsets.UTF_8)) {
                ...
                stockpile.capacity = o.get("capacity").getAsDouble();
                stockpile.usedCapacity = o.get("usedCapacity").getAsDouble();
