    ...
    private void processExchanges() {
        ...
        for (Map.Entry<String, CulturalExchange> e : activeExchanges.entrySet()) {
            ...
            if (now >= exchange.endTime) {
                ...
                // Apply bonuses
                plugin.getCultureService().developCulture(exchange.nationA, exchange.culturalBonus * 0.1);
                plugin.getCultureService().developCulture(exchange.nationB, exchange.culturalBonus * 0.1);
                // Improve reputation
                try {
                    Nation a = nationManager.getNationById(exchange.nationA);
                    Nation b = nationManager.getNationById(exchange.nationB);
                    if (a != null && b != null) {
                        plugin.getDiplomacySystem().setReputation(a, b, 1);
                        plugin.getDiplomacySystem().setReputation(b, a, 1);
                    }
                } catch (Exception ignored) {}
            }
        }
