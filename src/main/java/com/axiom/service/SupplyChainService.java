...
public class SupplyChainService {
    ...
    public synchronized void processChains() {
        ...
        for (SupplyChain sc : chains.values()) {
            ...
            String sourceNationId = cityGrowthEngine.getCitiesOf("").stream()
                .filter(c -> c.getId().equals(sc.sourceCityId))
                .findFirst()
                .map(c -> c.getNationId())
                .orElse(null);
            if (sourceNationId == null) continue;
            double amount = sc.ratePerHour / 60.0;
            if (resourceService.consumeResource(sourceNationId, sc.resourceType, amount)) {
                String targetNationId = cityGrowthEngine.getCitiesOf("").stream()
                    .filter(c -> c.getId().equals(sc.targetCityId))
                    .findFirst()
                    .map(c -> c.getNationId())
                    .orElse(null);
                if (targetNationId != null) {
                    resourceService.addResource(targetNationId, sc.resourceType, amount);
                }
            }
        }
    }
    ...
    public synchronized Map<String, Object> getSupplyChainStatistics(String nationId) {
        ...
        for (SupplyChain sc : chains.values()) {
            com.axiom.model.City sourceCity = cityGrowthEngine.getCity(sc.sourceCityId);
            com.axiom.model.City targetCity = cityGrowthEngine.getCity(sc.targetCityId);
