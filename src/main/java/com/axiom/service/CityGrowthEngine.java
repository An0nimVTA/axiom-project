            ...
            org.bukkit.Bukkit.getScheduler().runTask(plugin, () -> {
                for (UUID citizenId : n.getCitizens()) {
                    ...
                    if (citizen != null && citizen.isOnline()) {
                        ...
                        plugin.getVisualEffectsService().sendActionBar(citizen, "§b🏙️ Город '" + name + "' создан!");
                        // Blue particles
    ...
    private void loadAll() {
        ...
        for (File f : files) {
            try (Reader r = new InputStreamReader(Files.newInputStream(f.toPath()), StandardCharsets.UTF_8)) {
                ...
                City c = new City(o.get("id").getAsString(), o.get("name").getAsString(), o.get("nationId").getAsString(), o.get("centerChunk").getAsString());
                c.setLevel(o.get("level").getAsInt());
                c.setPopulation(o.get("population").getAsInt());
