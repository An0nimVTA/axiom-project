...
public class MilitaryIntelligenceService {
    ...
    public synchronized String collectIntelligence(String collectorId, String targetId, String type, double cost) {
        ...
        switch (type.toLowerCase()) {
            case "military":
                report.data.put("strength", plugin.getMilitaryService().getMilitaryStrength(targetId));
            ...
            case "economic":
                ...
                break;
            case "diplomatic":
                report.data.put("allies", target.getAllies().size());
                report.data.put("enemies", target.getEnemies().size());
                break;
            case "technological":
                report.data.put("education", plugin.getEducationService().getEducationLevel(targetId));
                break;
        }
        
        reports.put(reportId, report);
        collector.setTreasury(collector.getTreasury() - cost);
        collector.getHistory().add("Собрана разведка: " + type + " от " + target.getName());
        try {
