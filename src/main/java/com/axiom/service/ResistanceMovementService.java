...
public class ResistanceMovementService {
    ...
    public synchronized void establishMovement(String territoryKey, String targetNationId) {
        ...
        Nation n = plugin.getNationManager().getNationById(targetNationId);
        if (n != null) {
    ...
    private void loadAll() {
        ...
        for (File f : files) {
            try (Reader r = new InputStreamReader(Files.newInputStream(f.toPath()), StandardCharsets.UTF_8)) {
                ...
                ResistanceMovement rm = new ResistanceMovement();
                rm.territoryKey = o.get("territoryKey").getAsString();
                rm.targetNationId = o.get("targetNationId").getAsString();
                rm.supportLevel = o.get("supportLevel").getAsDouble();
                rm.establishedAt = o.get("establishedAt").getAsLong();
    ...
    public synchronized Map<String, Object> getResistanceStatistics(String nationId) {
        ...
        for (ResistanceMovement rm : movements.values()) {
            if (rm.targetNationId.equals(nationId)) {
    ...
    public synchronized List<ResistanceMovement> getTargetingMovements(String nationId) {
        return movements.values().stream()
            .filter(rm -> rm.targetNationId.equals(nationId))
    ...
    public synchronized Map<String, Object> getGlobalResistanceMovementStatistics() {
        ...
        for (ResistanceMovement rm : movements.values()) {
            ...
            averageSupportByNation.put(rm.targetNationId,
                (averageSupportByNation.getOrDefault(rm.targetNationId, 0.0) + rm.supportLevel) / 2.0);
        }
