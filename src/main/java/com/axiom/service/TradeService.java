...
public class TradeService {
    ...
    private final Map<String, Set<String>> sanctions = new HashMap<>(); // sanctioner -> sanctioned nations
    ...
    public synchronized String imposeSanction(String sanctionerId, String targetId) throws IOException {
        ...
        sanctions.computeIfAbsent(sanctionerId, k -> new HashSet<>()).add(targetId);
        saveSanctions();
        ...
        org.bukkit.Bukkit.getScheduler().runTask(plugin, () -> {
            ...
            for (UUID citizenId : s.getCitizens()) {
            ...
            for (UUID citizenId : t.getCitizens()) {
                org.bukkit.entity.Player citizen = org.bukkit.Bukkit.getPlayer(citizenId);
                if (citizen != null && citizen.isOnline()) {
    ...
    public synchronized boolean hasSanction(String nationA, String nationB) {
        return sanctions.getOrDefault(nationA, Collections.emptySet()).contains(nationB) ||
               sanctions.getOrDefault(nationB, Collections.emptySet()).contains(nationA);
    }
    ...
    private void loadSanctions() {
        File f = new File(treatiesDir, "sanctions.json");
        if (!f.exists()) return;
        try (Reader r = new InputStreamReader(Files.newInputStream(f.toPath()), StandardCharsets.UTF_8)) {
            ...
            for (var entry : o.entrySet()) {
                ...
                for (var e : arr) set.add(e.getAsString());
                sanctions.put(entry.getKey(), set);
            }
        } catch (Exception ignored) {}
    }

    private void saveSanctions() throws IOException {
        File f = new File(treatiesDir, "sanctions.json");
        JsonObject o = new JsonObject();
        for (var entry : sanctions.entrySet()) {
    ...
    public synchronized Map<String, Object> getTradeStatistics(String nationId) {
        ...
        stats.put("sanctionsImposed", sanctions.getOrDefault(nationId, Collections.emptySet()).size());
        
        // Count nations that have sanctioned this nation
        int sanctionedBy = 0;
        for (Set<String> sanctioned : sanctions.values()) {
            if (sanctioned.contains(nationId)) {
                sanctionedBy++;
            }
        }
        ...
        if (plugin.getStatisticsService() != null) {
            StatisticsService.NationStats stat = plugin.getStatisticsService().getStats(nationId);
            stats.put("totalTradeVolume", stat.totalTradeVolume);
        }
    ...
    public synchronized String removeSanction(String sanctionerId, String targetId) throws IOException {
        Set<String> sanctioned = sanctions.get(sanctionerId);
        if (sanctioned == null || !sanctioned.contains(targetId)) {
            return "Санкция не найдена.";
        }
        
        sanctioned.remove(targetId);
        if (sanctioned.isEmpty()) {
            sanctions.remove(sanctionerId);
        }
        saveSanctions();
    ...
    public synchronized double calculateTradeIncome(String nationId) {
        ...
        double baseIncome = n.getClaimedChunkKeys().size() * 1.0;
    ...
    public synchronized Map<String, Object> getGlobalTradeStatistics() {
        ...
        for (Map.Entry<String, Set<String>> entry : sanctions.entrySet()) {
            String sanctioner = entry.getKey();
            Set<String> sanctioned = entry.getValue();
            
            sanctionsByNation.put(sanctioner, sanctioned.size());
            totalSanctions += sanctioned.size();
            
            for (String target : sanctioned) {
                sanctionedByNation.put(target, sanctionedByNation.getOrDefault(target, 0) + 1);
            }
        }
